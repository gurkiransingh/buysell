...
class Cart extends React.Component {
    constructor(props) {
        ...
        this.state = {
            nothingYet: this.props.cartItems.length === 0,
    ...
    fetchUpdated(item, size) {
        Axios.post('/deleteItemFromCart', {custId: this.props.userId, itemId: item._id, size: size})
            .then((res) => {
                console.log(res);
                if(res.data === true) {
    ...
    render() {
        return (
            <div>
                <div className = {this.state.nothingYet ? '' : 'hide'}>
                <NothingYet />
                </div>
               
                <div className={'cart-container ' + (this.state.nothingYet ? "hide" : '')}>
                    <div className='items-container'>
                    {
                    this.state.cartItems.map((v,i) => {
                        return (
                        <Item fetchUpdated={this.fetchUpdated} items={this.props.cartItems} size={this.props.sizes[i]} item={v} key={i} history={this.props} fromCart={true}/>
                            )   
                        })
                    } 
                    </div>
